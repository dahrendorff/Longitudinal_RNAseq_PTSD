#Data was quality controlled using FastQC (Andrews, 2010), version 0.11.9.
#Sequencing adaptors have been trimmed from the reads. 
#The resulting reads were aligned to the reference genome GRCh38 (ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_24/GRCh38.primary_assembly.genome.fa.gz).
#After alignment of the reads the quantification of expression on the gene level was 
#facilitated with the use of the Featurecounts function of the R-package Subreads, version 2.0.0 (Liao, Smyth & Shi , 2019) on R version 4.0.5. (03, 2021)
#using Bioconductor version 3.15. The resulting read count data from the Featurecounts table was filtered by the number of counts and the low read counts were subjected to removal prior to downstream analysis
...
#countmatrix = featurecounts file "cleaned_combined_countmatrix_remove_dub.txt"

countMatrix<-read.table("cleaned_combined_countmatrix _remove_dub.csv",header = TRUE,
...
#metadata with cellproportions. Cellproportions were previously merged into the metadatafile

metadata<-read.csv("dnhs_dream_metadata_input.csv")
head(metadata)
table(metadata$Sex)
table(metadata$ptsd)
table(metadata$CD4T)
...
METADATA <-read.csv("dnhs_dream_metadata_input.csv")
df_cellFractions = read.csv("Cell_proportions_NSCLC_PBMC_both_timepoints.csv")
...
write.csv(METADATA1,"METADATA_log_transf_prop.csv")


Metadata1 = read.csv("METADATA_log_transf_prop_both.csv")
...
write.table(top.table, file = "TopTableOutput_ptsd_cases_vs_control_nowave.txt", row.names = F, sep = "\t", quote = F)
...
write.table(top.table, file = "TopTableOutput_ptsd_wave.txt1", row.names = F, sep = "\t", quote = F)
...
df=read.csv("TopTableOutput_ptsd_cases_vs_control_final.csv")
...
df1=read.csv("TopTableOutput_ptsd_wave_final.csv")
